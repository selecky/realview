...
class BookDetailBloc extends Bloc<BookDetailEvent, BookDetailState> {
  ...
  BookDetailBloc({required this.getBookDetailUseCase}) : super(const BookDetailStateLoading()) {
  ...
  Future<void> _onGetBookDetailEvent(
    ...
    emit(const BookDetailStateLoading());
    final GetBookDetailResult result = await getBookDetailUseCase.call(params: event.isbn13);
    switch (result) {
      case final ResultSuccess success:
        emit(BookDetailStateSuccess(bookDetail: success.bookDetail));
      case ResultFailure():
