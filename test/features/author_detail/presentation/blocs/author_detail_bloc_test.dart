...
void main() {
  ...
  blocTest<AuthorDetailBloc, AuthorDetailState>(
    'emits [AuthorDetailStateLoading, AuthorDetailStateSuccess] when GetAuthorsEvent succeeds',
    build: () {
      when(
        () => mockGetAuthorDetailUseCase.call(params: testId),
      ).thenAnswer((_) async => const ResultSuccess(authorDetail: testAuthorDetail));
      return authorDetailBloc;
    },
    act: (bloc) => bloc.add(const GetAuthorDetailEvent(authorId: testId)),
    expect:
        () => [
